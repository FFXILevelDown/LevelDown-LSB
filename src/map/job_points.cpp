...
{
    if (IsJobPointExist(jpType) && (m_PChar->GetMLevel() == 75 || m_PChar->GetMLevel() == 99) && m_PChar->GetMJob() == JobPointsCategoryIndexByJpType(jpType))
    {
...
void RefreshGiftMods(CCharEntity* PChar)
{
    ...
    } // End level 99 spell safety check
